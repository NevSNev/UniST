...
class Transformer(nn.Module):
    ...
    def forward(self, content , style_images, id_loss='transfer', tab = None):
        if (id_loss == 'transfer'):
            ...
            B,N,D = enc_content.size()
            enc_content = enc_content.view(b,t,N,D)

            image_part = torch.split(enc_content,[t//2,t//2],dim=1)[0]
            video_part = torch.split(enc_content,[t//2,t//2],dim=1)[1]

            image_part = image_part.reshape(-1,N,D)
            video_part = video_part.reshape(-1,N,D)

            image_encode = self.encoder_frames(image_part)
            video_encode = self.encoder_frames(video_part)

            image_cross = self.image_cross_video(image_encode,video_encode)
            video_cross = self.video_cross_image(video_encode,image_encode)

            image_cross = image_cross.reshape(-1,t//2,N,D)
            video_cross = video_cross.reshape(-1,t//2,N,D)
            enc_content = torch.cat([image_cross,video_cross],dim = 1)
            enc_content = enc_content.view(-1,N,D)
            ...
            dec_output = self.style_decoder(enc_content, enc_style_image)
        ...
        if (id_loss == 'content'):
            ...
            content = content.view(b*t, c, h, w)
            enc_content = self.encoder(content)
            enc_content = rearrange(enc_content,'b c h w ->b (h w) c')
            b, t, c, h, w = style_images.size()
            frames_style = style_images.view(b*t, c, h, w)
            enc_style = self.encoder(frames_style)
            enc_style = rearrange(enc_style,'b c h w ->b (h w) c')
        ...
        if (id_loss == 'style'):
            ...
            enc_content = self.encoder(content)
            b,c,h,w = enc_content.size()
            enc_content = rearrange(enc_content,'b c h w ->b (h w) c')
            enc_content = self.encoder_frames(enc_content)

            enc_style = self.encoder(style_images)
            enc_style = rearrange(enc_style,'b c h w ->b (h w) c')
            enc_style = self.encoder_style(enc_style)


            dec_output = self.style_decoder(enc_content, enc_style)
            style_result = self.decoder(dec_output)
            return style_result
